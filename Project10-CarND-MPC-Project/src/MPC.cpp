...
size_t N = 10;  // higher N leads to instability along turns
double dt = 0.08;
...
public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Minimize the use of actuators. (scale factor important)
    // scale  = 1 causes erratic movements
    // scale = 100 slows down the car
    for (int i = 0; i < N - 1; i++) {
    ...
    // (how smooth the actuations are, higher scale causes wobble)
    for (int i = 0; i < N - 2; i++) {
      fg[0] += 50000 *
